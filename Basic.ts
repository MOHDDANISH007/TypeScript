let myName : string  = "Mohammad Danish"
console.log(myName);


let isBoolean : boolean = false;

const value : boolean = isBoolean===false? true : false;
...
// The any type in TypeScript is a special type that essentially turns off TypeScript's type checking for a variable. When you use any, you’re telling TypeScript: "I don’t know what type this is, so don’t check it." This can be useful in some cases, but it also removes the safety and benefits of TypeScript's type system.
...
// In TypeScript, Function Parameter Annotation refers to the process of specifying the types of the parameters that a function accepts. This allows TypeScript to enforce type checking for the arguments passed to the function, ensuring that the function is called with the correct types. It helps improve the reliability and maintainability of code by providing clear expectations of the types that a function can accept.
...
function greet(name:string, age:number) : void{
    console.log(`Hello ${name} , I hope you are good and your age is ${age}`);
}

const v = greet("danish", 21);
console.log(v);
...
const v2 = greet1("Mohammad Mobasshir Ali", 21);
console.log(v2);


const addTwoNumber = (x:number, y:number) : number=>{
    return x + y;
}
...
const anyTypeInArrow = (xAny : any , yAny:any) : void =>{
    console.log(`${xAny} and ${yAny}`);
}
...
const restOperator = (...xyz:any) :void=>{
    xyz.forEach((value:any)=>{
        console.log(value);
    })
    const sum = xyz.reduce((acc:any, curr:any)=>{
        return acc + curr;
    },0)
    console.log(sum)
}
...
const returnArray = (x:number, y:number) :any[]=>{
    console.log(`Number 1st is ${x} And 2nd Number is ${y}`);
    return [1,2,3,4,5];
}
